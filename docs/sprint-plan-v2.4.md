...
- **S8 / G3 (stable URL)** — ✅ code shipped; **domain acquired: `lifengrace.com`** — cutover runbook in DEPLOYMENT.md "Custom domain cutover": CloudFront distribution in front of the Function URL, optional custom domain via CDK context (`customDomain`/`hostedZoneId`/`hostedZoneName`). Deploy + `APP_BASE_URL` switch + Google redirect URI re-registration remain for a credentialed session.
...
- **S10 / G8 (Mobile Phase 1)** — ✅ code complete in `mobile/` (see mobile-app-plan Progress for detail); remaining for S10: real-device run, Sentry wiring (G9), store-account enrollment (G10, human).
- **User request (pre-mobile): more Scripture per topic** — ✅ code shipped + locally verified: static catalog expanded to 10 curated verses per topic (70 total); new "Find more verses" flow backed by a shared TopicVerse library (main DB — `prisma db push` needed on RDS) that serves previously fetched verses at zero AI cost and only asks the LLM when the library is exhausted, excluding + deduping against every known reference; AI-suggested verses are marked in the UI and usable for prayer generation via server-side lookup.
